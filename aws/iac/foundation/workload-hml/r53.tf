...
resource "aws_route53_resolver_rule_association" "outbound_rule_association_galgo_use1" {
  provider         = aws.use1
  resolver_rule_id = data.terraform_remote_state.shared.outputs.resolver_rule_galgo_use1
  vpc_id           = module.vpc_use1.vpc_id
}

resource "aws_route53_resolver_rule_association" "outbound_rule_association_galgo_sae1" {
  provider         = aws.sae1
  resolver_rule_id = data.terraform_remote_state.shared.outputs.resolver_rule_galgo_sae1
  vpc_id           = module.vpc_sae1.vpc_id
}
