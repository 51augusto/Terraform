module "vpc_sae1" {
  ...
  intra_subnets  = ["10.94.20.0/22", "10.94.24.0/22", "10.94.28.0/22"]
  public_subnets = ["10.94.0.0/22", "10.94.4.0/22", "10.94.8.0/22"]
...
/*
resource "aws_ec2_transit_gateway_vpc_attachment" "audit_sae1" {
  provider           = aws.sae1
  ...
  depends_on         = [module.vpc_sae1]
}

resource "aws_route" "audit_sae1_internet" {
  provider               = aws.sae1
