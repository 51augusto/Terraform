module "vpc_use1" {
  ...
  intra_subnets  = ["10.83.20.0/22", "10.83.24.0/22", "10.83.28.0/22"]
  public_subnets = ["10.83.0.0/22", "10.83.4.0/22", "10.83.8.0/22"]
...
/*
resource "aws_ec2_transit_gateway_vpc_attachment" "log_archive_use1" {
  provider           = aws.use1
...
resource "aws_route" "log_archive_use1_internet" {
  provider               = aws.use1
