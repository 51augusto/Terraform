module "vpc_use1" {
  ...
  intra_subnets  = ["10.87.20.0/22", "10.87.24.0/22", "10.87.28.0/22"]
  public_subnets = ["10.87.0.0/22", "10.87.4.0/22", "10.87.8.0/22"]
...
resource "aws_ec2_transit_gateway_vpc_attachment" "sandbox_use1" {
  provider           = aws.use1
...
resource "aws_route" "sandbox_use1_internet" {
  provider               = aws.use1
...
resource "aws_db_subnet_group" "rds_use1" {
  provider   = aws.use1
