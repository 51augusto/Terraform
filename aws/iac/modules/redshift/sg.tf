resource "aws_security_group" "sg" {
  name                   = local.sg_name
  description            = "Security group do Redshift ${local.cluster_name}"
  vpc_id                 = var.vpc_id
...
resource "aws_security_group_rule" "default_ingress_sg_rule" {
  ...
  type              = "ingress"
  description       = "Liberacao de acesso a porta do banco para o range de IPs da conta shared"
  from_port         = 5439
  to_port           = 5439
  protocol          = "tcp"
  cidr_blocks = concat(
...
resource "aws_security_group_rule" "internal_network" {
  security_group_id = aws_security_group.sg.id
  description       = "Liberacao de acesso a porta do banco para ranges de IP da rede interna da ANBIMA"
  type              = "ingress"
  from_port         = 5439
  to_port           = 5439
  protocol          = "tcp"
